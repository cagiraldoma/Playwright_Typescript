...
export class AssignLeavePage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.leaveTypeInput = page.locator(
      '//div[@class="oxd-input-group oxd-input-field-bottom-space"]//div[@class="oxd-select-wrapper"]',
    );
    this.leaveTypeList = page.locator(".oxd-select-option");
    this.fromDateInput = page
      .locator('//div[@class="oxd-date-input"]/input')
      .first();
    this.toDateInput = page
      .locator('//div[@class="oxd-date-input"]/input')
      .last();
    this.partialDaysInput = page.locator(
      '//div[@class="oxd-select-wrapper"]/div[@role="listbox"]/div',
    );
    this.durationButton = page.locator(
      '(//div[@class="oxd-select-text oxd-select-text--active"])[2]',
    );
    this.partialDaysButton = page.locator(
      '//div[@class="oxd-select-text oxd-select-text--active"]//div[contains(text(),"-- Select --")]',
    );
    this.durationList = page.locator(
      '//div[@class="oxd-select-dropdown --positon-bottom"]/div/span',
    );
    this.commentsTextArea = page.locator(
      '//div/textarea[@class="oxd-textarea oxd-textarea--active oxd-textarea--resize-vertical"]',
    );
  }
